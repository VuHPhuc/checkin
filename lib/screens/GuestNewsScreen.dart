...
class GuestNewsScreen extends StatefulWidget {
  // Màn hình hiển thị tin tức cho khách
    ...
    if (response.statusCode == 200) {
      ...
      for (var element in newsElements) {
        ...
        if (!link.startsWith('http://') && !link.startsWith('https://')) {
          // Nếu link không có http hoặc https thì thêm vào
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        LoginScreen()), // Chuyển sang màn hình đăng nhập
              );
            },
          ),
        ],
                            ...
                            children: [
                              Text(item['title'] ?? '',
                                  style: const TextStyle(
                                      ...
                                      fontSize:
                                          18)), // Hiển thị tiêu đề tin tức
                              const SizedBox(height: 8),
                              Text(item['date'] ?? '',
                                  style: const TextStyle(
                                      fontSize:
                                          14)), // Hiển thị ngày đăng tin tức
                            ],
                          ),
                        ),
                      ],
