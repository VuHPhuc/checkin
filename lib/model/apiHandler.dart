...
class APIHandler {
  final String baseUrl = 'http://192.168.1.46:5176/api';
  final Dio dio = Dio();
  ...
  Future<List<Task>> getUserTasks(
    ...
    try {
      final response = await dio.get(
        '$baseUrl/tasks?userId=$userId&startDate=${DateFormat('yyyy-MM-dd').format(startDate)}&endDate=${DateFormat('yyyy-MM-dd').format(endDate)}',
        options: Options(headers: {'Content-Type': 'application/json'}),
      );
  ...
  Future<bool> insertTask(Task task) async {
    try {
      ...
      if (response.statusCode == 201) {
        // Thêm task thành công (thường trả về 201 Created)
        return true;
      } else {
        // Xử lý lỗi khi thêm task
        print('Error inserting task: ${response.statusCode}');
        return false;
      }
    } catch (e) {
      // Xử lý lỗi
